...
class Talk(object):
    ...
    @loggedIn
    def sendTagLocation(self, to, text, contentMetadata={}):
        msg = Message()
        msg.to, msg._from = to, self.profile.mid
        msg.text = text
        msg.contentType, msg.contentMetadata = 15, contentMetadata
        location = Location()
        location.address = "Tag All Message"
        location.phone = None
        location.latitude = 0.0
        location.longitude = 0.0
        location.title = "LT Tech Bot"
        msg.location = location
        if to not in self._messageReq:
            self._messageReq[to] = -1
        self._messageReq[to] += 1
    ...
    @loggedIn
    def replyMention(self, msgid, to, text="", mids=[]):
        arrData = ""
        arr = []
        mention = "@LT_Tech_Bot"
        if mids == []:
        ...
        if "@!" in text:
            ...
            arr.append(arrData)
            textx += mention + str(text)
        self.sendReplyMessage(msgid, to, textx, {'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)
